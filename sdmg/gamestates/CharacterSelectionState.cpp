...
namespace sdmg
{
	namespace gamestates
	{
		void CharacterSelectionState::init(GameBase& game)
		{
			...
			_characters = new std::map<std::string, nlohmann::json>();
			_lockedCharacters = new std::map<std::string, nlohmann::json>();
			...
			for (int i = 0; i < _slots->size(); i++)
			{
				...
				game.getEngine()->getInputEngine()->getMouse().setClickAction(x, SELECTED_CHARACTER_BOX_YPOS + 48, SELECTED_CHARACTER_BOX_WIDTH, SELECTED_CHARACTER_BOX_HEIGHT - 48, (std::function<void()>)[&, i]
				{
					(*_slots)[i].clear();
				});
				...
				{
					if (_slotKeyInput->at(key) >= _keys->size() - 1)
					{
						_slotKeyInput->at(key) = 0;
					}
					else
					{
						_slotKeyInput->at(key)++;
					}
				});
			}
			...
			_menu->addMenuTextItem("Back to game modes", (std::function<void()>)[&]
			{
				changeState(game, GameModeState::getInstance());
			});
			...
			for (size_t i = 0; i < characterList.size(); i++)
			{
				...
				if (!ifile.fail())
				{
					try
					{
						auto json = engine::util::FileManager::getInstance().loadFileContentsAsJson("assets/characters/" + characterFolder + "/data");

						if (ProgressManager::getInstance().isUnlockedCharacter(characterFolder))
						{

							_characters->insert({ characterFolder, json });
							...
							game.getEngine()->getDrawEngine()->loadText("s_" + characterFolder + "_name", json["name"].get<std::string>(), { 255, 255, 255 }, "trebucbd", 12);
						...
						else
						{
							...
							_lockedCharacters->insert({ characterFolder, json });
						}
					}
					catch (...)
					{
		...
		{
			int numPlayerCount = std::count_if(_slots->begin(), _slots->end(), [](std::string slot)
			{
				return !slot.empty();
			});
